...
## 현재 데이터 상태 (2026-09-11 기준)

- 전체 166마리 (ohrsa.net 스크레이핑 159 + 수동등록 7: 꼬미/꼬비/꼬꼬비/꼬비오/만모로스/보르비스/도라비스)
- 근사치(approx) 펫: 없음 (전부 정밀 매칭 — "기존 방식" 기준. "세분화 방식"으론
  일부 근사치 있음, 아래 항목 참고)
- 미지원(ok=false): 없음 — 꼬비도 2026-09-11에 실제 초기치/성장률 확인되어
  정밀 매칭(origin=[24,30,18,30], k=20)으로 전환 완료.
- **사이트 원본 표기 오타는 `data/overrides.json`으로 고정**: 93(만모,
  순발력 22→2), 155(타키온, 공격력 100→10 — 2026-09-11 확인, approx이던 게
  정밀 매칭으로 바뀜). 다음에도 새 펫이 approx로 뜨거나 fit_resid가 유난히
  크면(수백~수천 단위) 표기 오타를 먼저 의심하고, 의심되는 자리를 사용자가
  불러주는 값으로 바꿔 재계산해서 확인하는 절차를 반복하면 된다.
