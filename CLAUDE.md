...
5. 정보가 불확실한 펫은 억지로 계산하지 말고 `growthS`/`initS`를 `[0,0,0,0]`,
   `ok:false`로 비워서 "계산 미지원"으로 표시한다. 나중에 실제 수치가
   확인되면 `common.calibrate_pet`으로 재계산해서 채우고, `originAlt`/`kAlt`
   /`confMain`/`confAlt`/`originDevMain`/`devFlagMain` 등 파생 필드도 전부
   같이 재계산해야 한다(꼬비가 2026-09-11에 이렇게 전환된 사례 — 스크립트
   패턴은 그 커밋 참고).

## 현재 데이터 상태 (2026-09-26 기준)

- 전체 184마리 (자동 동기화로 신펫 계속 추가됨 + 수동등록 3: 만모로스/보르비스/
  도라비스 — 꼬미/꼬비/꼬꼬비/꼬비오는 ohrsa.net에 정식으로 올라오면서
  자동 승격 로직으로 이미 정식 데이터 교체 완료됨)
- 근사치(approx) 펫: 없음 (전부 정밀 매칭)
- 미지원(ok=false): 없음
- **사이트 원본 표기 오타는 `data/overrides.json`으로 고정**: 93(만모,
  순발력 22→2), 155(타키온, 공격력 100→10 — 2026-09-11 확인), 195(한복밍코,
  순발력 110→10, 끝자리 0 오탈자 — 2026-09-26 확인, k=35/fit_resid 9705.9→
  k=27/fit_resid 0.73로 정밀 매칭 전환). 다음에도 새 펫이 approx로 뜨거나
  fit_resid가 유난히 크면(수백~수천 단위) 표기 오타를 먼저 의심하고,
  의심되는 자리를 사용자가 불러주는 값으로 바꿔 재계산해서 확인하는 절차를
  반복하면 된다(이번엔 111 자리 대신 11/10 두 후보를 다 계산해 비교하는
  방식으로 확정함 — 11은 여전히 근사치, 10만 정밀 매칭이라 10으로 확정).
