...
- **`common.confidence_score(calib)`**: (2026-09-10 재설계, method 인자 없어짐)
  "이 origin/k로 성장률·초기치를 다시 계산했을 때 실제 표기값과 얼마나
  벌어지는가"를 그대로 신뢰도 근거로 쓴다 — `growth_resid`/`fit_resid`를
  각각 `1/(1+resid/scale)` 꼴 팩터로 바꿔 곱하고, 근사치면 0.7 추가 감점,
  `CONFIDENCE_HARD_CAP=97`로 100은 절대 못 넘게 막는다. **이전엔 main/ext에
  임의의 구조 가중치(0.85/0.45)를 곱했었는데, 사용자가 "그 가중치가 근거
  있냐"고 문제 제기해서 폐기함.**
  - 재설계 전 검증한 사실(계속 유효): RANKS(main)의 growth_resid는 142마리
    전량 사실상 0(중앙값 0.0, 최대 0.000391)인 반면, RANKS_EXT(ext)는 두
    표가 갈리는 21마리 **전부 예외 없이** growth_resid가 main보다 크다
    (0.0002~0.006). ext가 후보 구간을 3개(main은 1개) 갖고 있어 유리해
    보일 수 있는데도 여전히 못 맞춘다 — "구간을 더 쪼개서 생긴 착시"가
    아니라 우리 서버의 실제 성장률 데이터가 main 구조를 직접 지지한다는
    뜻. 그래서 별도 가중치 없이 residual 크기 자체가 이 차이를 자연스럽게
    반영하게 둠(위 함수 설명 참고).
